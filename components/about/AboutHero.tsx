    ...
    <section className={`w-full px-4 mt-12 py-12 sm:px-6 lg:px-8 lg:py-20 text-primary-heading ${isRTL ? 'text-right' : ''}`}>
      <div className="mx-auto max-w-[1140px]">
        <div className="grid gap-6 md:grid-cols-2 lg:gap-8 items-stretch">
          <div className="flex flex-col justify-between rounded-[36px] bg-primary-surface border border-primary-border p-6 sm:p-10 lg:p-12 shadow-soft">
            <div className="space-y-4">
              <h1 className="text-3xl font-bold tracking-tight text-primary-heading sm:text-4xl lg:text-[42px] lg:leading-[1.15]">
                <span dangerouslySetInnerHTML={{ __html: t.raw('hero.feature_title') }} />
              </h1>

              <div className="h-[3px] w-12 rounded-full bg-brand" />

              <p className="pt-2 text-sm leading-relaxed text-text-muted sm:text-base opacity-90">
                {t('hero.feature_body')}
              </p>
            </div>

            <div className={`mt-8 inline-flex flex-wrap items-center gap-2 rounded-full p-1.5 max-w-max ${isRTL ? 'flex-row-reverse' : ''}`}>
              <Link
                href="/plans"
          ...
          <div className="relative min-h-[440px] md:min-h-full overflow-hidden rounded-[40px] text-white flex items-center justify-center">
            ...
            <motion.div
              ...
              className={`absolute top-18 md:top-4 right-0 md:right-4 z-20 p-3 rounded-2xl`}
            >
