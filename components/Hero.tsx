...
export interface HeroFloatingItem {
  icon: ReactNode;
  label?: string;
}
...
export interface HeroProps {
  ...
  imageSrc: string;
  imageSizes?: string;
  floatingText?: string | { __html: string };
  floatingTextIsHtml?: boolean;
  floatingElements?: ReactNode[];
  className?: string;
  showFloatingIcons?: boolean;
  floatingIcons?: HeroFloatingItem[];
}
...
const Hero = ({
  ...
  const renderTitle = () => {
    if (titleIsHtml && typeof title === 'object' && '__html' in title) {
      return (
        <span 
  ...
  const renderFloatingText = () => {
    ...
    if (floatingTextIsHtml && typeof floatingText === 'object' && '__html' in floatingText) {
      return (
        <p 
          className="text-[13px] font-medium leading-snug" 
          dangerouslySetInnerHTML={floatingText} 
        />
      );
    }
    return <p className="text-[13px] font-medium leading-snug">{floatingText as string}</p>;
  };
  ...
  return (
    <section className={`w-full px-4 mt-12 py-12 sm:px-6 lg:px-8 lg:py-20 text-primary-heading ${isRTL ? 'text-right' : ''} ${className}`}>
            ...
            <div className="w-[92%] h-auto aspect-square relative drop-shadow-[0_20px_40px_rgba(0,0,0,0.3)] select-none pointer-events-none z-10">
              ...
              {floatingText && (
                <motion.div
                  ...
                  className={`absolute top-[18%] ${isRTL ? 'right-[6%]' : 'left-[6%]'} bg-white text-primary-heading px-5 py-3 rounded-[20px] shadow-lg max-w-[210px] pointer-events-auto absolute z-50`}
                >
                  {renderFloatingText()}
                </motion.div>
              )}
